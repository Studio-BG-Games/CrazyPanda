...
namespace AmazingAssets.AdvancedDissolveEditor
{
    ...
    static public class MaterialEditor
    {
        ...
        static public void Init(MaterialProperty[] props)
        {
            ...
            if (iconError == null)
                iconError = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(Utilities.GetPathToTheAssetInstallationtFolder(), "Editor", "Icons", "erroricon.png"), typeof(Texture2D));

            if (iconWarning == null)
                iconWarning = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(Utilities.GetPathToTheAssetInstallationtFolder(), "Editor", "Icons", "warningicon.png"), typeof(Texture2D));
                        ...
                        using (new AmazingAssets.EditorGUIUtility.EditorGUIUtilityLabelWidth(0))
                        {
                            using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginVertical(EditorStyles.helpBox))
                            {
                                ...
                                using (new AmazingAssets.EditorGUIUtility.EditorGUIIndentLevel(1))
                                {
                                    ...
                                    {
                                        using (new AmazingAssets.EditorGUIUtility.GUIEnabled(globalControlID == AdvancedDissolveKeywords.GlobalControlID.None))
                                        {
                                    ...
                                    using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(GUI.skin.settings.selectionColor))
                                    {
                                        using (new AmazingAssets.EditorGUIUtility.GUIEnabled(hasBakedKeywords ? false : true))
                                        {
        ...
        static void DrawVectorWithLock(UnityEditor.MaterialEditor materialEditor, string label, MaterialProperty property, bool isVector3)
        {
            ...
            using (new AmazingAssets.EditorGUIUtility.GUIEnabled(propIsLocked ? false : true))
            {
        ...
        {
            using (new AmazingAssets.EditorGUIUtility.EditorGUIUtilityLabelWidth(defaultLabelWidth - 6))
            {
        ...
        static void DrawTextureWrapMode(UnityEditor.MaterialEditor editor, Rect drawRect, MaterialProperty textureProperty)
        {
            ...
            if (textureProperty.textureValue != null)
            {
                ...
                using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(wrapMode == TextureWrapMode.Clamp || wrapMode == TextureWrapMode.MirrorOnce ? Color.red : Color.white))
                {
                        ...
                        using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginVertical(EditorStyles.helpBox))
                        {
                            ...
                            for (int i = 0; i < suitableScriptWithoutMaterial.Count; i++)
                            {
                                using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginHorizontal())
                                {
                                    ...
                                    using (new AmazingAssets.EditorGUIUtility.GUIBackgroundColor(cutoutGeometricCount == suitableScriptWithoutMaterial[i].count ? Color.white : Color.green))
                                    {
